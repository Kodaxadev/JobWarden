# Changelog archive — v68 and earlier

Versions are the service-worker cache id (`jobwarden-vNN`), shown in **Settings → About**.
This file holds the entries that rolled off [`CHANGELOG.md`](../CHANGELOG.md) when it reached
the 400-line cap in AGENTS.md. Newest first; the current cycle lives in the main file.
